...
fn try_finalize<S: Storage, A: Api, Q: Querier>(
    ...
    if !no_bids {
        ...
        if state.tokens_consigned && !state.is_completed {
            ...
            let length = bid_list.len();
            if length >= 2 {
                let highest_amount:u128 = (bid_list[length-1].bid.amount).into();
                let second_highest_amount:u128 = (bid_list[length-2].bid.amount).into();
                let return_amt =  highest_amount - second_highest_amount;
                if let Some(winning_bid) = bid_list.pop() {
                    cos_msg.push(
                        state
                            .bid_contract
                            .transfer_msg(state.seller.clone(),Uint128::from(second_highest_amount))?,
                    );
                    ...
                    cos_msg.push(state.bid_contract.transfer_msg(
                        deps.api.human_address(&winning_bid.bidder)?,
                        Uint128::from(return_amt),
                    )?);
                    state.currently_consigned = Uint128(0);
                    update_state = true;
                    winning_amount = Some(Uint128::from(highest_amount));
